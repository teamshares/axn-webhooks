...
* **Auto-`dimension` by vendor:** the adapter stamps `dimension :vendor, <registered name>` (`:twilio`, `:codat`, …) onto the pipeline from the mount/definition name — a bounded low-cardinality facet ideal for grouping/filtering Datadog metrics + traces + exception reports by vendor, with **zero per-handler boilerplate**. (`dimension`, not `tag`: vendor is a small known set. Event-type could be a higher-cardinality `tag` if wanted.)
...
7. **Auto-`dimension :vendor, <name>`** stamped by the adapter for per-vendor observability grouping (ties to [PRO-2818](https://linear.app/teamshares/issue/PRO-2818/axn-datadog-observability-axn-overview-dashboard)).
